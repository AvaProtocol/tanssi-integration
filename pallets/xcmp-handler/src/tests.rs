...
fn get_instruction_set_local_currency_instructions() {
    ...
    new_test_ext().execute_with(|| {
        ...
        let descend_location: Junctions = AccountIdToLocation::convert(ALICE).try_into().unwrap();
...
fn transact_in_local_chain_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            events(),
            [RuntimeEvent::XcmpHandler(
                crate::Event::XcmTransactedLocally
            )]
        );
    });
}
...
fn transact_in_target_chain_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(XcmpHandler::transact_in_target_chain(
            destination.clone(),
            target_instructions
        ));
        assert_eq!(
        ...
        assert_eq!(
            events(),
            [RuntimeEvent::XcmpHandler(crate::Event::XcmSent {
                destination
            })]
        );
    });
