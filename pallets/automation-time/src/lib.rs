...
pub mod pallet {
    ...
    pub type TaskOf<T> = types::Task<AccountOf<T>>;
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn schedule_xcmp_task(
            ...
            schedule_fee: Box<VersionedLocation>,
            ...
            let destination_location =
                Location::try_from(*destination.clone()).map_err(|()| Error::<T>::BadVersion)?;
            let schedule_fee_location =
                Location::try_from(*schedule_fee.clone()).map_err(|()| Error::<T>::BadVersion)?;

            let execution_fee_payment: AssetPayment = *execution_fee.clone();
            let execution_fee_location =
                Location::try_from(execution_fee_payment.clone().asset_location)
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn get_current_time_slot() -> Result<UnixTime, DispatchError> {
            ...
            let diff_to_slot = now
                .checked_rem(T::SlotSizeSeconds::get())
                .ok_or(ArithmeticError::Overflow)?;
            Ok(now
                .checked_sub(diff_to_slot)
                .ok_or(ArithmeticError::Overflow)?)
        }
        ...
        pub fn trigger_tasks(max_weight: Weight) -> Weight {
            ...
            let max_update_weight: Weight = Weight::from_parts(
                T::UpdateQueueRatio::get().mul_floor(weight_left.ref_time()),
                0,
            );
        ...
        pub fn update_scheduled_task_queue(
            ...
            if current_time_slot != last_time_slot {
                ...
                if let Some(ScheduledTasksOf::<T> {
        ...
        pub fn run_xcmp_task(
            ...
            let fee_asset_location = Location::try_from(fee.asset_location);
        ...
        fn insert_scheduled_tasks(
            ...
            with_transaction(
                || -> storage::TransactionOutcome<Result<TaskIdV2, DispatchError>> {
                    for time in execution_times.iter() {
                        let mut scheduled_tasks =
                            Self::get_scheduled_tasks(*time).unwrap_or_default();
                        if scheduled_tasks
                            .try_push::<T>(task_id.clone(), task)
                            .is_err()
                        {
        ...
        pub fn calculate_schedule_fee_amount(
            ...
            let fee = if schedule_fee_location == Location::default() {
        ...
        pub fn ensure_supported_execution_fee_location(
            exeuction_fee_location: &Location,
            destination: &Location,
        ) -> Result<(), DispatchError> {
            let exeuction_fee = Asset {
                id: exeuction_fee_location.clone().into(),
                fun: Fungibility::Fungible(0),
            };
            let reserve = T::ReserveProvider::reserve(&exeuction_fee)
                .ok_or(Error::<T>::InvalidAssetLocation)?;
            if reserve != Location::here() && &reserve != destination {
