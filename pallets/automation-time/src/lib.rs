...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn schedule_xcmp_task(
			...
			let destination_location = Location::try_from(*destination.clone())
				.map_err(|()| Error::<T>::BadVersion)?;
			let schedule_fee_location = Location::try_from(*schedule_fee.clone())
				.map_err(|()| Error::<T>::BadVersion)?;
	...
	impl<T: Config> Pallet<T> {
		...
		fn insert_scheduled_tasks(
			...
			with_transaction(|| -> storage::TransactionOutcome<Result<TaskIdV2, DispatchError>> {
				for time in execution_times.iter() {
					...
					if scheduled_tasks.try_push::<T>(task_id.clone(), task).is_err() {
		...
		pub fn ensure_supported_execution_fee_location(
			...
			let exeuction_fee =
				Asset { id: exeuction_fee_location.clone().into(), fun: Fungibility::Fungible(0) };
