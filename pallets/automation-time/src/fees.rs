	...
	TR: TakeRevenue,
{
		...
		match &self.execution_fee {
			Some(exec_fee) if exec_fee.is_local => {
				...
				if exec_fee_location == schedule_fee_location {
					...
					Self::ensure_can_withdraw(self, exec_fee.asset_location.clone(), fee)?;
				} else {
					Self::ensure_can_withdraw(
						self,
						self.schedule_fee.asset_location.clone(),
						self.schedule_fee.amount,
					)?;
					Self::ensure_can_withdraw(self, exec_fee.asset_location.clone(), exec_fee.amount)?;
				}
			},
			_ => {
				Self::ensure_can_withdraw(
	...
	fn withdraw_fee(&self) -> Result<(), DispatchError> {
		...
		if !self.schedule_fee.amount.is_zero() {
			let currency_id = T::CurrencyIdConvert::convert(self.schedule_fee.asset_location.clone())
			...
			TR::take_revenue(Asset {
				id: self.schedule_fee.asset_location.clone().into(),
		...
		if let Some(execution_fee) = &self.execution_fee {
			if execution_fee.is_local {
				...
				let currency_id = T::CurrencyIdConvert::convert(execution_fee.asset_location.clone())
					.ok_or("InconvertibleMultilocation")?;
	...
	pub fn new(
		...
		let execution_fee = match action.clone() {
			Action::XCMP { execution_fee, instruction_sequence, .. } => {
				let location = Location::try_from(execution_fee.asset_location)
