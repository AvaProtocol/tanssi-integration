...
pub const NATIVE_LOCATION: Location = Location {
    parents: 0,
    interior: Here,
};
...
pub fn get_moonbase_asset_location() -> Location {
    Location {
        parents: 1,
        interior: X2([Parachain(1000u32), PalletInstance(3u8)].into()),
    }
}
...
pub fn new_test_ext(state_block_time: u64) -> sp_io::TestExternalities {
    let genesis_storage = system::GenesisConfig::<Test>::default()
        .build_storage()
        .unwrap();
...
pub fn schedule_dynamic_dispatch_task(
    ...
    assert_ok!(AutomationTime::schedule_dynamic_dispatch_task(
        ...
        ScheduleParam::Fixed {
            execution_times: scheduled_times
        },
        Box::new(call),
    ));
    last_task_id()
