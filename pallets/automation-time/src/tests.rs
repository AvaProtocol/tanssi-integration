...
use staging_xcm::{
    latest::{prelude::*, Junction::Parachain, Junctions::*},
    VersionedLocation,
};
...
fn calculate_expected_xcmp_action_schedule_fee(
    schedule_fee_location: Location,
    ...
    if schedule_fee_location == Location::default() {
    ...
    new_test_ext(START_BLOCK_TIME).execute_with(|| {
        ...
        assert_noop!(
            AutomationTime::schedule_dynamic_dispatch_task(
                ...
                ScheduleParam::Fixed {
                    execution_times: vec![SCHEDULED_TIME + 1]
                },
                Box::new(call)
            ),
    ...
    new_test_ext(START_BLOCK_TIME + 1_000 * SLOT_SIZE_SECONDS * 3).execute_with(|| {
        assert_noop!(
            AutomationTime::schedule_dynamic_dispatch_task(
                RuntimeOrigin::signed(AccountId32::new(ALICE)),
                ScheduleParam::Fixed {
                    execution_times: vec![SCHEDULED_TIME]
                },
        ...
        assert_noop!(
            AutomationTime::schedule_dynamic_dispatch_task(
                RuntimeOrigin::signed(AccountId32::new(ALICE)),
                ScheduleParam::Fixed {
                    execution_times: vec![SCHEDULED_TIME - SLOT_SIZE_SECONDS]
                },
                Box::new(frame_system::Call::remark { remark: vec![12] }.into())
            ),
            Error::<Test>::PastTime,
...
fn schedule_transfer_with_dynamic_dispatch() {
    new_test_ext(START_BLOCK_TIME).execute_with(|| {
        ...
        condition.insert(
            ...
            SCHEDULED_TIME.to_string().into_bytes(),
        );
...
fn will_emit_task_completed_event_when_task_completed() {
    new_test_ext(START_BLOCK_TIME).execute_with(|| {
        ...
        assert_ok!(AutomationTime::schedule_dynamic_dispatch_task(
            ...
            ScheduleParam::Fixed {
                execution_times: vec![SCHEDULED_TIME, next_execution_time]
            },
            Box::new(call),
        ));
...
fn will_emit_task_completed_event_when_task_failed() {
    new_test_ext(START_BLOCK_TIME).execute_with(|| {
        ...
        condition.insert(
            "timestamp".as_bytes().to_vec(),
            next_execution_time.to_string().into_bytes(),
        );
...
fn calculate_xcmp_action_schedule_fee_amount_with_different_schedule_fees_works() {
    new_test_ext(START_BLOCK_TIME).execute_with(|| {
        get_asset_fee_per_second_config()
            .into_iter()
            .for_each(|fee| {
                let num_of_execution = generate_random_num(1, 20);
...
fn calculate_xcmp_action_schedule_fee_amount_with_absolute_or_relative_native_schedule_fee_works() {
    new_test_ext(START_BLOCK_TIME).execute_with(|| {
        ...
        let action_relative = create_xcmp_action(XcmpActionParams {
            schedule_fee: Location::new(0, Here),
            ..XcmpActionParams::default()
        });
        ...
        let expected_schedule_fee_amount =
            calculate_expected_xcmp_action_schedule_fee(Location::new(0, Here), num_of_execution);
...
fn calculate_xcmp_action_schedule_fee_amount_with_different_destination_returns_same_result() {
    new_test_ext(START_BLOCK_TIME).execute_with(|| {
        ...
        let action_diffrent_dest = create_xcmp_action(XcmpActionParams {
            destination: Location::new(1, X1([Parachain(3000)].into())),
            ..XcmpActionParams::default()
        });
...
fn calculate_xcmp_action_schedule_fee_amount_with_unknown_schedule_fees_fails() {
    new_test_ext(START_BLOCK_TIME).execute_with(|| {
        let num_of_execution = generate_random_num(1, 20);
        let unknown_schedule_fee = Location {
            parents: 1,
            interior: Parachain(4000).into(),
        };
        let action = create_xcmp_action(XcmpActionParams {
...
fn schedule_xcmp_task_and_check_encoded_call_success() {
    new_test_ext(START_BLOCK_TIME).execute_with(|| {
		...
		let destination: Box<VersionedLocation> = Box::new(Location::new(1, X1([Parachain(PARA_ID)].into())).into());
		let schedule_fee: Box<VersionedLocation> = Box::new(NATIVE_LOCATION.into());
...
fn schedule_duplicates_errors() {
    new_test_ext(START_BLOCK_TIME).execute_with(|| {
        ...
        assert_noop!(
            AutomationTime::schedule_task(&task2),
...
fn schedule_time_slot_full_rolls_back() {
    new_test_ext(START_BLOCK_TIME).execute_with(|| {
        ...
        let task_id1 = schedule_task(
            ALICE,
            vec![SCHEDULED_TIME + SLOT_SIZE_SECONDS * 2],
            vec![2, 4, 5],
        );
...
fn cancel_works_for_multiple_executions_scheduled() {
    new_test_ext(START_BLOCK_TIME).execute_with(|| {
        ...
        assert_eq!(
            AutomationTime::get_account_task(owner.clone(), task_id1.clone()),
            None
        );
        if AutomationTime::get_scheduled_tasks(SCHEDULED_TIME).is_some() {
...
fn cancel_works_for_an_executed_task() {
    new_test_ext(START_BLOCK_TIME).execute_with(|| {
        ...
        assert_eq!(AutomationTime::get_scheduled_tasks(SCHEDULED_TIME), None);
        assert_eq!(
            AutomationTime::get_scheduled_tasks(SCHEDULED_TIME + SLOT_SIZE_SECONDS),
            None
        );
...
mod run_dynamic_dispatch_action {
    ...
    fn cannot_decode() {
        new_test_ext(START_BLOCK_TIME).execute_with(|| {
            ...
            assert_eq!(
                error,
                Some(DispatchError::from(Error::<Test>::CallCannotBeDecoded))
            );
        })
    }
...
fn trigger_tasks_limits_missed_slots() {
    new_test_ext(START_BLOCK_TIME).execute_with(|| {
        ...
        let missing_task_id2 = schedule_task(
            ALICE,
            vec![SCHEDULED_TIME - SLOT_SIZE_SECONDS * 2],
            vec![50],
        );
        let missing_task_id3 = schedule_task(
            ALICE,
            vec![SCHEDULED_TIME - SLOT_SIZE_SECONDS * 3],
            vec![50],
        );
        let missing_task_id4 = schedule_task(
            ALICE,
            vec![SCHEDULED_TIME - SLOT_SIZE_SECONDS * 4],
            vec![50],
        );
        let missing_task_id5 = schedule_task(
            ALICE,
            vec![SCHEDULED_TIME - SLOT_SIZE_SECONDS * 5],
            vec![50],
        );
...
fn trigger_tasks_completes_all_tasks() {
    new_test_ext(START_BLOCK_TIME).execute_with(|| {
        ...
        condition.insert(
            "timestamp".as_bytes().to_vec(),
            LAST_BLOCK_TIME.to_string().into_bytes(),
        );
        ...
        assert_eq!(0, AutomationTime::get_task_queue().len());
        assert_eq!(
            AutomationTime::get_account_task(AccountId32::new(ALICE), task_id1),
            None
        );
        assert_eq!(
            AutomationTime::get_account_task(AccountId32::new(ALICE), task_id2),
            None
        );
    })
}
...
fn trigger_tasks_completes_some_tasks() {
    new_test_ext(START_BLOCK_TIME).execute_with(|| {
        ...
        assert_eq!(1, AutomationTime::get_task_queue().len());
        assert_eq!(
            AutomationTime::get_account_task(owner.clone(), task_id1),
            None
        );
...
fn on_init_runs_tasks() {
    new_test_ext(START_BLOCK_TIME).execute_with(|| {
        ...
        let task_id4 = add_task_to_task_queue(
            ALICE,
            vec![70],
            vec![SCHEDULED_TIME],
            create_dynamic_dispatch_remark_action(message_two.clone()),
        ...
        let task_id5 = add_task_to_task_queue(
            ALICE,
            vec![80],
            vec![SCHEDULED_TIME],
        ...
        assert_eq!(
            AutomationTime::get_account_task(owner.clone(), task_id2),
            None
        );
        assert_eq!(
            AutomationTime::get_account_task(owner.clone(), task_id3),
            None
        );
        assert_eq!(
            AutomationTime::get_account_task(owner.clone(), task_id4),
            None
        );
        assert_ne!(
            AutomationTime::get_account_task(owner.clone(), task_id5.clone()),
            None
        );
        assert_eq!(AutomationTime::get_task_queue().len(), 1);
        assert_eq!(AutomationTime::get_missed_queue().len(), 0);
        ...
        assert_eq!(AutomationTime::get_account_task(owner, task_id5), None);
        assert_eq!(AutomationTime::get_task_queue().len(), 0);
        assert_eq!(AutomationTime::get_missed_queue().len(), 0);
    })
}
...
fn on_init_check_task_queue() {
    new_test_ext(START_BLOCK_TIME).execute_with(|| {
        ...
        assert_eq!(AutomationTime::get_task_queue().len(), 5);
        assert_eq!(AutomationTime::get_missed_queue().len(), 0);
        ...
        assert_eq!(
            ...
            [
                ...
                RuntimeEvent::System(frame_system::pallet::Event::Remarked {
                    sender: owner.clone(),
                    hash: BlakeTwo256::hash(vec![4].as_slice()),
                }),
                RuntimeEvent::AutomationTime(crate::Event::TaskExecuted {
                    who: owner.clone(),
                    task_id: tasks[4].clone(),
                ...
                RuntimeEvent::AutomationTime(crate::Event::TaskTriggered {
                    who: owner.clone(),
                    task_id: tasks[5].clone(),
                    condition: condition.clone(),
                }),
                RuntimeEvent::System(frame_system::pallet::Event::Remarked {
                    sender: owner.clone(),
                    hash: BlakeTwo256::hash(vec![5].as_slice()),
                }),
                RuntimeEvent::AutomationTime(crate::Event::TaskExecuted {
                    who: owner.clone(),
                    task_id: tasks[5].clone(),
                ...
                RuntimeEvent::AutomationTime(crate::Event::TaskTriggered {
                    who: owner.clone(),
                    task_id: tasks[6].clone(),
                    condition: condition.clone(),
                }),
                RuntimeEvent::System(frame_system::pallet::Event::Remarked {
                    sender: owner.clone(),
                    hash: BlakeTwo256::hash(vec![6].as_slice()),
                }),
                RuntimeEvent::AutomationTime(crate::Event::TaskExecuted {
                    who: owner.clone(),
                    task_id: tasks[6].clone(),
                ...
                RuntimeEvent::AutomationTime(crate::Event::TaskTriggered {
                    who: owner.clone(),
                    task_id: tasks[7].clone(),
                    condition: condition.clone(),
                }),
                RuntimeEvent::System(frame_system::pallet::Event::Remarked {
                    sender: owner.clone(),
                    hash: BlakeTwo256::hash(vec![7].as_slice()),
                }),
                RuntimeEvent::AutomationTime(crate::Event::TaskExecuted {
                    who: owner.clone(),
                    task_id: tasks[7].clone(),
