...
pub struct FeeHandler<T: Config, TR> {
    owner: T::AccountId,
    pub schedule_fee_location: Location,
    ...
    TR: TakeRevenue,
{
    ...
    fn can_pay_fee(&self) -> Result<(), DispatchError> {
        let fee = self
            .schedule_fee_amount
            .saturating_add(self.execution_fee_amount);
        ...
        let currency_id = T::CurrencyIdConvert::convert(self.schedule_fee_location.clone())
            .ok_or("IncoveribleLocation")?;
        ...
        match T::MultiCurrency::withdraw(currency_id.into(), &self.owner, fee) {
            Ok(_) => {
                TR::take_revenue(Asset {
                    id: self.schedule_fee_location.clone().into(),
