...
use crate::{
    mock::*, AccountStats, Action, AssetPayment, Error, StatType, Task, TaskIdList, TaskStats,
    Tasks,
};
...
use staging_xcm::latest::{prelude::*, Junction::Parachain, Location};
...
fn test_update_asset_price_increase_round() {
    new_test_ext(START_BLOCK_TIME).execute_with(|| {
        ...
        Timestamp::set_timestamp(
            (START_BLOCK_TIME_1HOUR_AFTER_IN_SECOND * 1000)
                .try_into()
                .unwrap(),
        );
        assert_ok!(AutomationPrice::update_asset_prices(
...
fn test_schedule_xcmp_task_ok() {
    new_test_ext(START_BLOCK_TIME).execute_with(|| {
        ...
        assert_eq!(
            task.chain,
            CHAIN1.to_vec(),
            "created task has different chain id"
        );
        assert_eq!(
            task.asset_pair.0, ASSET1,
            "created task has wrong asset pair"
        );
...
fn test_sweep_expired_task_works() {
    new_test_ext(START_BLOCK_TIME).execute_with(|| {
        ...
        let destination = Location::new(1, Parachain(para_id));
        let schedule_fee = Location::default();
        let execution_fee = AssetPayment {
            asset_location: Location::new(1, Parachain(para_id)).into(),
            amount: MOCK_XCMP_FEE,
        };
        ...
        Timestamp::set_timestamp(START_BLOCK_TIME.saturating_add(3_600_000_u64));
        ...
        assert_eq!(
            u128::try_from(Tasks::<Test>::iter().count()).unwrap(),
            expired_task_gen + 1
        );
        ...
        AutomationPrice::sweep_expired_task(Weight::from_parts(remain_weight, 0));
...
fn test_sweep_expired_task_partially() {
    new_test_ext(START_BLOCK_TIME).execute_with(|| {
        ...
        // Now we set timestamp to a later point
        Timestamp::set_timestamp(START_BLOCK_TIME.saturating_add((3600 + 6 * 10) * 1000));
...
fn test_shift_tasks_movement_through_price_changes() {
    new_test_ext(START_BLOCK_TIME).execute_with(|| {
        ...
        assert_eq!(
            ...
            vec![(creator.clone(), task_id1.clone())]
        );
        ...
        let _ = AutomationPrice::update_asset_prices(
            RuntimeOrigin::signed(creator.clone()),
            vec![CHAIN2.to_vec()],
            vec![EXCHANGE1.to_vec()],
            vec![ASSET1.to_vec()],
...
fn test_emit_event_when_execute_tasks() {
    new_test_ext(START_BLOCK_TIME).execute_with(|| {
        ...
        let _ = AutomationPrice::validate_and_schedule_task(task.clone());
...
fn test_decrease_task_count_when_execute_tasks() {
    new_test_ext(START_BLOCK_TIME).execute_with(|| {
        ...
        let _ = AutomationPrice::validate_and_schedule_task(task1.clone());
        let _ = AutomationPrice::validate_and_schedule_task(task2);
...
fn test_expired_task_not_run() {
    new_test_ext(START_BLOCK_TIME).execute_with(|| {
        ...
        Timestamp::set_timestamp(START_BLOCK_TIME.saturating_add(7_200_000_u64));
        AutomationPrice::run_tasks(
            vec![(task.owner_id.clone(), task.task_id.clone())],
            100_000_000_000.into(),
        );
...
fn test_price_move_against_target_price_skip_run() {
    new_test_ext(START_BLOCK_TIME).execute_with(|| {
        ...
        assert_last_event(RuntimeEvent::AutomationPrice(
            crate::Event::PriceAlreadyMoved {
                owner_id: task.owner_id.clone(),
                task_id: task.task_id,
...
fn test_cancel_task_works() {
    new_test_ext(START_BLOCK_TIME).execute_with(|| {
        ...
        let _ = AutomationPrice::cancel_task(
...
fn test_delete_asset_ok() {
    new_test_ext(START_BLOCK_TIME).execute_with(|| {
        ...
        let _ = AutomationPrice::delete_asset(
            RawOrigin::Root.into(),
            CHAIN1.to_vec(),
            EXCHANGE1.to_vec(),
            ASSET1.to_vec(),
