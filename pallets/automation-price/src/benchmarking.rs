...
fn schedule_xcmp_task<T: Config>(para_id: u32, owner: T::AccountId, call: Vec<u8>) {
    let _ = AutomationPrice::<T>::schedule_xcmp_task(
        ...
        "gt".as_bytes().to_vec(),
        vec![2000],
        Box::new(Location::new(1, Parachain(para_id)).into()),
...
benchmarks! {
    ...
    run_xcmp_task {
        ...
        let fee = AssetPayment { asset_location: Location::new(1, Parachain(para_id)).into(), amount: 1000u128 };
    }: {
