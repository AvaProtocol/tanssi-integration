...
use frame_support::{
    pallet_prelude::{
        DispatchError, DispatchResult, Encode, Get, IsType, MaxEncodedLen,
        MaybeSerializeDeserialize, Member, NMapKey, Parameter, StorageDoubleMap, StorageMap,
        StorageNMap, StorageValue, StorageVersion, Twox64Concat, ValueQuery,
    },
    traits::Currency,
    transactional,
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn delete_asset(
            ...
            if let Some(_asset_info) = Self::get_asset_registry_info(key) {
                ...
                Self::deposit_event(Event::AssetDeleted {
                    chain,
                    exchange,
                    asset1,
                    asset2,
        ...
        pub fn schedule_xcmp_task(
            ...
            let destination =
                Location::try_from(*destination).map_err(|()| Error::<T>::BadVersion)?;
            let schedule_fee =
                Location::try_from(*schedule_fee).map_err(|()| Error::<T>::BadVersion)?;
            ...
            match task.action.clone() {
                Action::XCMP {
                    ...
                    let asset_location = asset_location
                        .reanchored(
                            &Location::new(1, Parachain(T::SelfParaId::get().into())),
                    ...
                    if instruction_sequence == InstructionSequence::PayThroughSovereignAccount
                        && asset_location != Location::new(0, Here)
                    {
